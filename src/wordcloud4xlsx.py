...
class WordCloud4XLSX:
    ...
    def ReadXlsx(self):
        ...
        for title in titles:
            ...
            temp_title   = temp_title.replace('.', ' ')
            print(f'{temp_title}')
            titles_list.append(temp_title)
        total_titles    = ' '.join(titles_list)
        print(f'debug - {total_titles}')
        temp_word_counter   = Counter(total_titles.split())
        self.m_word_counter = {}
        for key in temp_word_counter:
            temp_key        = str(key)
            temp_key.replace(',', '')
            temp_value      = temp_word_counter[key]
            self.m_word_counter[temp_key]   = temp_value
        #self.m_word_counter = Counter(' '.join(titles).split())
        print(f'debug - {self.m_word_counter}')
    ...
    def MakeWordCloud(self):
        ...
        if 'kr' == self.m_lang:
            wordcloud = WordCloud(width=800,
                                  ...
                                  font_path=self.m_korean_font).generate_from_frequencies(self.m_word_counter)
        elif 'persian' == self.m_lang:
            wordcloud = WordCloud(width=800,
                                  height=800,
                                  background_color='white',
                                  font_path=self.m_persian_font).generate_from_frequencies(self.m_word_counter)
        plt.figure(figsize=(10,10))
        plt.imshow(wordcloud, interpolation='bilinear')
        plt.axis('off')
        plt.savefig(f'{self.m_output_prefix}.png')
        plt.show()
